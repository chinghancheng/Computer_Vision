...
# 組合 + 顯示圖片
img_show_rotate = np.hstack((img, img_rotate))
img_show_trans = np.hstack((img, img_rotate_trans))
while True:
    cv2.imshow('Rotate 45, scale 0.5', img_show_rotate)
    cv2.imshow('Translate x+100, y-50', img_show_trans)
    k = cv2.waitKey(0)
    if k == 27:
